...
where
    ...
    FResume: FnMut(&str, &str, bool) -> Result<()>,
{
    run_with_impl(
        config,
...
fn run_with_impl(
    ...
    resume_conversation: &mut dyn FnMut(&str, &str, bool) -> Result<()>,
) -> Result<()> {
    ...
    resume_conversation(&config.codex_bin, selected.id(), config.dry_run)
}
...
pub fn resume_conversation(codex_bin: &str, conversation_id: &str, dry_run: bool) -> Result<()> {
    if dry_run {
        let mut stdout = io::stdout();
        return write_dry_run_output(&mut stdout, codex_bin, conversation_id);
    }

    resume_subprocess_conversation(codex_bin, conversation_id)
}
...
fn resume_command_status(
        ...
        .status()
    {
        ...
        Err(error) => Err(error).context(format!(
            "failed to execute `{codex_bin} resume {conversation_id}`"
        )),
    }
}
...
fn resume_subprocess_conversation(codex_bin: &str, conversation_id: &str) -> Result<()> {
    let status = match classify_resume_status(resume_command_status(codex_bin, conversation_id)) {
