...
where
    ...
    FResume: FnOnce(&str, &str, &str, bool) -> anyhow::Result<()>,
{
    ...
    resume(codex_bin, selected.id(), selected.cwd.as_str(), dry_run)
}
...
fn app_query_matches_ignoring_punctuation() {
    ...
    app.query = "invoicerepair".to_string();
    app.refresh_visible();
...
fn normalize_search_text_removes_case_and_punctuation() {
    assert_eq!(
        normalize_search_text("Invoice-Repair / TEST"),
        "invoicerepairtest".to_string()
    );
}
...
fn run_picker_app_resumes_selected_conversation() {
    let resumed = Arc::new(Mutex::new(None::<(String, String, String, bool)>));
    ...
    let result = run_picker_app(
        ...
        move |bin, id, cwd, dry_run| {
            resumed_capture
                .lock()
                .unwrap_or_else(|_| unreachable!("resume mutex should lock"))
                .replace((bin.to_string(), id.to_string(), cwd.to_string(), dry_run));
            Ok(())
        },
    );
...
fn run_picker_app_rejects_empty_conversation_lists() {
    let result = run_picker_app(
        ...
        |_, _, _, _| unreachable!("resume should not run"),
    );
...
fn run_with_propagates_include_subagents_loader_errors() {
    ...
    let error = run_with(
        ...
        |_bin, _id, _cwd, _dry_run| unreachable!("loader errors should stop before resume"),
    )
    .err()
...
fn resume_conversation_executes_subprocess_and_reports_failures() {
    ...
    let success_result = resume_conversation(
        success_script.to_str().unwrap_or(""),
        "thread-1",
        &cwd,
        false,
    );
